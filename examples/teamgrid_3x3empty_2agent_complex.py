...
def carrying(env: TestEnv, agent: int):
    # this is the ball position not the agent position
    if env.agents[agent].carrying is None:
        # print(f"agent position: {env.agents[agent].cur_pos}")
        # print(f"ball position: {env.agents[agent].front_pos}, type: {env.agents[agent].front_pos}")
        if np.array_equal(env.agents[agent].front_pos, np.array([3, 3])):
            return "D"
        else:
            return "I"
    else:
...
def make_carry_obj_dfa():
    ...
    dfa.states = MoveObjStates()
    dfa.add_state(dfa.states.init, pickup_obj)
...
if __name__ == '__main__':
    ...
    mas = MAS(env=env, models=models, dfas=dfas, one_off_reward=one_off_reward,
                   num_tasks=num_tasks, num_agents=num_agents, render=False, e=e, c=c)
